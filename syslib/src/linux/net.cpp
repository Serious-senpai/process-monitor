...
namespace
{
    io::Result<_net_impl::NativeSocketAddr> sockname(int socket, int (*getname)(int, sockaddr *, socklen_t *))
    {
        // First try to get the address family
        sockaddr_storage storage;
        socklen_t len = sizeof(storage);

        if (getname(socket, reinterpret_cast<sockaddr *>(&storage), &len) == -1)
        {
        ...
        {
            sockaddr_in *addr_v4 = reinterpret_cast<sockaddr_in *>(&storage);
        ...
        {
            sockaddr_in6 *addr_v6 = reinterpret_cast<sockaddr_in6 *>(&storage);
            return io::Result<_net_impl::NativeSocketAddr>::ok(
                _net_impl::NativeSocketAddr::v6(_net_impl::NativeSocketAddrV6::from_sockaddr(*addr_v6)));
        }

        return io::Result<_net_impl::NativeSocketAddr>::err(
            io::Error(io::ErrorKind::InvalidData, "Unknown address family"));
    }

    io::Result<std::monostate> setsockopt_int(int socket, int level, int optname, int value)
    {
        if (setsockopt(socket, level, optname, &value, sizeof(value)) == -1)
        {
    ...
    io::Result<int> getsockopt_int(int socket, int level, int optname)
    {
        int value = 0;
        socklen_t len = sizeof(value);
        if (getsockopt(socket, level, optname, &value, &len) == -1)
        {
...
namespace _net_impl
{
    ...
    NativeTcpStream::NativeTcpStream(int socket)
    ...
    io::Result<NativeTcpStream> NativeTcpStream::connect_v4(const NativeSocketAddrV4 &addr)
    {
        ...
        if (::connect(sock, reinterpret_cast<struct sockaddr *>(&sockaddr), sizeof(sockaddr)) == -1)
        {
            int error = errno;
            close(sock);
    ...
    io::Result<std::optional<std::chrono::milliseconds>> NativeTcpStream::read_timeout() const
    {
        ...
        if (getsockopt(_socket, SOL_SOCKET, SO_RCVTIMEO, &tv, &len) == -1)
        {
        ...
        auto ms = tv.tv_sec * 1000 + tv.tv_usec / 1000;
        return io::Result<std::optional<std::chrono::milliseconds>>::ok(std::chrono::milliseconds(ms));
    }
    ...
    io::Result<std::optional<std::chrono::milliseconds>> NativeTcpStream::write_timeout() const
    {
        struct timeval tv;
        socklen_t len = sizeof(tv);
        if (getsockopt(_socket, SOL_SOCKET, SO_SNDTIMEO, &tv, &len) == -1)
        {
            return io::Result<std::optional<std::chrono::milliseconds>>::err(io::Error::last_os_error());
        }
        if (tv.tv_sec == 0 && tv.tv_usec == 0)
    ...
    {
        ssize_t result = recv(_socket, buffer.data(), buffer.size(), MSG_PEEK);
        if (result == -1)
        {
    ...
    {
        return setsockopt_int(_socket, IPPROTO_TCP, TCP_NODELAY, nodelay ? 1 : 0);
    }

    io::Result<bool> NativeTcpStream::nodelay() const
    {
        auto result = getsockopt_int(_socket, IPPROTO_TCP, TCP_NODELAY);
        if (result.is_err())
        {
            return io::Result<bool>::err(std::move(result).into_err());
        }
        return io::Result<bool>::ok(result.unwrap() != 0);
    }

    io::Result<std::monostate> NativeTcpStream::set_ttl(uint32_t ttl) const
    {
        return setsockopt_int(_socket, IPPROTO_IP, IP_TTL, static_cast<int>(ttl));
    }

    io::Result<uint32_t> NativeTcpStream::ttl() const
    {
        auto result = getsockopt_int(_socket, IPPROTO_IP, IP_TTL);
        if (result.is_err())
        {
            return io::Result<uint32_t>::err(std::move(result).into_err());
        }
        return io::Result<uint32_t>::ok(static_cast<uint32_t>(result.unwrap()));
    }

    io::Result<std::optional<io::Error>> NativeTcpStream::take_error() const
    {
        auto result = getsockopt_int(_socket, SOL_SOCKET, SO_ERROR);
        if (result.is_err())
        {
            return io::Result<std::optional<io::Error>>::err(std::move(result).into_err());
        }
        int error = result.unwrap();
        if (error == 0)
        {
    ...
    {
        ssize_t result = recv(_socket, buffer.data(), buffer.size(), 0);
        if (result == -1)
        {
            return io::Result<size_t>::err(io::Error::last_os_error());
        }
    ...
    {
        // Use MSG_NOSIGNAL to avoid SIGPIPE on broken connections (consistent with Rust)
        ssize_t result = send(_socket, buffer.data(), buffer.size(), MSG_NOSIGNAL);
        if (result == -1)
        {
            return io::Result<size_t>::err(io::Error::last_os_error());
        }
    ...
    NativeTcpListener::NativeTcpListener(int socket)
        : NonConstructible(NonConstructibleTag::TAG), _socket(socket)
    {
    }
    ...
    io::Result<NativeTcpListener> NativeTcpListener::bind_v4(const NativeSocketAddrV4 &addr)
    {
        ...
        sockaddr_in sockaddr = addr.to_sockaddr();
        if (::bind(sock, reinterpret_cast<struct sockaddr *>(&sockaddr), sizeof(sockaddr)) == -1)
        {
            int error = errno;
            close(sock);
            return io::Result<NativeTcpListener>::err(io::Error::from_raw_os_error(error));
    ...
    io::Result<std::pair<NativeTcpStream, NativeSocketAddr>> NativeTcpListener::accept() const
    {
        ...
        int client = ::accept(_socket, reinterpret_cast<sockaddr *>(&storage), &len);
        if (client == -1)
        {
    ...
    {
        return setsockopt_int(_socket, IPPROTO_IPV6, IPV6_V6ONLY, only_v6 ? 1 : 0);
    }

    io::Result<bool> NativeTcpListener::only_v6() const
    {
        auto result = getsockopt_int(_socket, IPPROTO_IPV6, IPV6_V6ONLY);
        if (result.is_err())
        {
            return io::Result<bool>::err(std::move(result).into_err());
        }
